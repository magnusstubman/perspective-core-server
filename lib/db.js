...
var _get = function(tableName, Clazz, id) {
  ...
  if (id) {
    ...
    rethinkdb.table(tableName).run(_connection, function(err, cur) {
      if (err) {
        apiLogger.error(err);
        deferred.reject(err);
        return;
...
var _update = function(tableName, Clazz, id, data) {
  var deferred = Q.defer();

  if (id) {
    rethinkdb.table(tableName).get(id).update(data).run(_connection, function(err, result) {
      if (err) {
        apiLogger.error(err);
        deferred.reject(err);
      }
  ...
  return function createRepository(tableName, Clazz) {
      ...
      return {
        ...
        update: function(id, data) {
          return _update(tableName, Clazz, id, data);
        }
      }
    }
};
